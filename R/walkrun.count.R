#' @title function to calculate the count values over series of epoches with specific count input parameters for walkrun data
#' @note This function would be only feasible for walkrun type of data. There are two dataset attached with the package "walkrun1" and "walkrun2". Please use `?walkrun1` or `?walkrun2` to check its format.
#' @param raw_data the dataset name, if it's a string, the function will try to load it using `data()` function otherwise will take it directly as a walkrun dataframe
...
#' @import plyr dplyr
#' @export
walkrun.count = function(raw_data = "walkrun1", 
  ...
  walkrun_data = eval(parse(text = raw_data))
  ...
  walkrun_count = walkrun_data %>% ddply(.(MPH, SUBJECT, SESSION, WEIGHT, LOCATION, SR), function(segment) {
  ...
  countCol = walkrun_count %>% colnames %>% str_detect(pattern = "ACCELATION") %>% which
  names(walkrun_count)[countCol] = "COUNT"
  
  walkrun_count = walkrun_count %>% subset(
    select = c(
      HEADER_TIME_STAMP,
      SUBJECT,
      SESSION,
  ...
  if(exists(x = actigraph_data)){
    ...
    walkrun_actigraph = eval(parse(text = actigraph_data))
    # select epoches
    walkrun_actigraph = walkrun_actigraph %>% subset(EPOCH %in% epoches)
    
    walkrun_count = walkrun_count %>% 
      subset(select = -1) %>%
      join(walkrun_actigraph, type = "inner") %>%
      subset(
        select = c(
          HEADER_TIME_STAMP,
          SUBJECT,
