...
#' @param walkrun_count output dataframe of walkrun.count function
...
#' @export
walkrun.plot = function(walkrun_count,
                         layout = SUBJECT ~ LOCATION,
                         series = "COUNT_TYPE",
                         sample_group = "SR",
                         scale = "z-score",
                         subset_condition = WEIGHT == "0" & EPOCH == "5 sec" & LOCATION %in% c("DominantWaist",
                                                                                               "DominantAnkle",
                                                                                               "NondominantWrist",
                                                                                               "DominantThigh")) {

  walkrun_count$LOW_BW = walkrun_count$LOW_BW %>% as.character
  walkrun_count$HIGH_BW = walkrun_count$HIGH_BW %>% as.character
  walkrun_count$RESAMPLE = walkrun_count$RESAMPLE %>% as.character
  
  # prepare melted data frame
  walkrun_scaled = walkrun_count %>%
    subset(select = -1) %>% ddply(.(EPOCH, RESAMPLE, FILTER_TYPE, LOW_BW, HIGH_BW, INTEGRATION), function(seg) {
      if (scale == "z-score") {
  ...
  walkrun_melted %>%
    ...
    ggplot(aes_string(x = "MPH", y = "COUNT", color = series)) +
