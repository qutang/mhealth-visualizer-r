...
walkrun1 = ldply(files, function(file){
  ...
  data = cbind(data, LOCATION = sensorLocation, SR = sr, MPH = speed, WEIGHT = weight, SUBJECT = subj_name, SESSION = session_name, stringsAsFactors = FALSE)
...
walkrun1_actigraph = ldply(files, function(file){
  ...
  data = cbind(data, INDEX = 1:nrow(data), EPOCH = epoch, LOCATION = sensorLocation, SR = sr, MPH = speed, WEIGHT = weight, SUBJECT = subj_name, SESSION = session_name, stringsAsFactors = FALSE)
