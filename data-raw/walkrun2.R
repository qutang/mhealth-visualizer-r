...
files = files[!str_detect(files, "segments") & !str_detect(files, "P4") & !str_detect(files, "P5") & !str_detect(files, "ad_converted")]
...
walkrun2 = files %>% ldply(function(file){
  ...
  data = file %>% 
    SensorData.importActigraphCsv(TRUE, FALSE, FALSE)
  
  SensorData.io.writeAsActigraphRaw(folder = file %>% dirname, sensorData = data, headerStr = SensorData.createActigraphCsvHeader(header$st, header$dt, header$sr, header$sn, header$fw, header$sw), custom_name = "ad_converted.csv")
  
  data = data %>%
    cbind(LOCATION = sensorLocation, SR = as.character(header$sr), MPH = NA, WEIGHT = "0", SUBJECT = subj_name, SESSION = session_name)
