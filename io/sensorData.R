...
SensorData.importCsv = function(filename){
  ...
  if(!grepl("csv", filename)) stop("Please make sure the raw data file is in csv or csv.gz format")
  # read.table supports csv.gz directly
  dat = read.table(filename, header = TRUE, sep = MHEALTH_CSV_DELIMITER, quote="\"", stringsAsFactors = FALSE)
...
SensorData.importBin= function(filename){
  ...
  dat = data.frame(MHEALTH_CSV_TIMESTAMP_HEADER = timestamp, 
...
# dat = SensorData.importBin("~/Projects/Android/DebugSPADESApp/SPADESTEST_01/FromWatch/data/MasterSynced/2015/06/14/00/GWatchRA326-ACCELERATION-CALIBRATED.34FCEF30A326.2015-06-14-00-00-00-473-M0400.baf")
